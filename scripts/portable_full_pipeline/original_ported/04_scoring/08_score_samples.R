...
# -------------------------
# PORTING NOTE: Configured paths are injected by the portable runner; scoring logic below is original.
# -------------------------
...
design_root <- Sys.getenv(
  "IMRS_PORTED_SCORING_DESIGN_ROOT",
  unset = file.path(project_root, "00_metadata", "verified_metadata", "scoring")
)
...
if (!dir.exists(counts_root)) stop("Missing configured counts root: ", counts_root)
