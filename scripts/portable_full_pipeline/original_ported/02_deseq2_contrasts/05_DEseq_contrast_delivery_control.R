...
# Example:
#   Rscript 05_DEseq_contrast_delivery_control.R "." "GSE167521"
# ============================================================
...
# -------------------------
# PORTING NOTE: Configured input roots are injected by the portable runner.
# -------------------------
args <- commandArgs(trailingOnly = TRUE)

project_root <- Sys.getenv(
  "IMRS_PORTED_OUTPUT_ROOT",
  unset = if (length(args) >= 1) args[1] else "."
)
dataset_only <- if (length(args) >= 2) args[2] else NA_character_
counts_root <- Sys.getenv(
  "IMRS_PORTED_COUNTS_ROOT",
  unset = file.path(project_root, "03_counts")
)
verified_metadata_root <- Sys.getenv(
  "IMRS_PORTED_METADATA_ROOT",
  unset = file.path(project_root, "00_metadata", "verified_metadata")
)

de_root <- file.path(project_root, "04_de")

split_design_root <- Sys.getenv(
  "IMRS_PORTED_SPLIT_DESIGN_ROOT",
  unset = file.path(verified_metadata_root, "splited")
)
