...
manual_paper_context_mapping <- tribble(
  ...
  "GSE178313", NA, "35132167", "10.1038/s41565-021-01030-y", 2022L, "Species-dependent in vivo mRNA delivery and cellular responses to nanoparticles", "Homo sapiens; Mus musculus", "GEO", "GSE178313", "lipid nanoparticle", "Species-dependent LNP delivery and cellular-response study; includes transcriptomic response to LNPs and humanized/mouse liver context.", "moderate_expected_signal", "Moderate positive IMRS is biologically plausible because the paper studies LNP-induced cellular responses, but interpretation is extended/context-shifted due to species/humanized context and 48 h timing.", "Extended validation, not primary acute validation.", "Species-transfer context; tissue/cell composition may differ from mouse acute anchors.",
  ...
  "GSE237068", NA, "38101397", "10.1016/j.cmet.2023.11.010", 2024L, "ChREBP is activated by reductive stress and mediates GCKR-associated metabolic traits", "Mus musculus", "GEO", "GSE237068", "metabolic/gene perturbation context; verify local metadata for vector details", "Metabolic/reductive-stress and ChREBP/GCKR-associated trait study. Not primarily a delivery-induced innate immune activation study.", "excluded_or_not_delivery_relevant", "If not scored, keep as metadata-only/excluded. If scored, require manual review because paper context is metabolic perturbation rather than clean delivery-induced innate activation.", "Metadata-audited but likely non-primary or excluded/contextual.", "Not a clean delivery-response dataset unless verified split metadata shows a relevant vector-delivery contrast.",
  "GSE262515", NA, "38902241", "10.1038/s41392-024-01871-8", 2024L, "Optimized RNA interference therapeutics combined with interleukin-2 mRNA for treating hepatitis B virus infection", "Homo sapiens; Mus musculus", "GEO", "GSE262515", "optimized therapeutic lipid nanoparticle / tLNP RNAi", "Therapeutic tLNP/RNAi context with cell-line and mouse tissue/liver arms. Current splits include tLNP/siHBV or tLNP/siNC treatment versus baseline_0 at 72 h.", "weak_but_biologically_explainable", "Weak or variable IMRS is explainable because the contrast is late, therapeutic, cargo/formulation-specific, and often baseline-vs-treatment rather than a clean shared-vehicle acute delivery contrast.", "Secondary support / non-primary evidence.", "Do not claim direct shared-vehicle comparison unless the split is explicitly tLNP/siHBV versus tLNP/siNC.",
...
design_only_rows <- tibble(dataset_id = design_only_expected) %>%
  ...
  transmute(
    ...
    in_vivo_or_in_vitro = NA_character_,
    disease_model = NA_character_,
    time_h = NA_real_,
    timepoint_original = NA_character_,
    treatment_group_raw = NA_character_,
    control_group_raw = NA_character_,
    treatment_group_clean = NA_character_,
    control_group_clean = NA_character_,
    condition_simple = NA_character_,
    delivery_platform = delivery_platform_manual,
    vector_or_formulation = delivery_platform_manual,
    cargo_or_transgene = NA_character_,
    ...
    direction_positive = NA,
    qc_pass = NA,
    inclusion_rationale = "Verified metadata folder/design exists, but no current scored split is present in the manuscript role table.",
    limitation = limitations_note,
    weak_score_explanation = NA_character_,
    reviewer_risk = "manual review if later used",
    notes = "Metadata-only dataset. No current IMRS result is assigned.",
    verification_status = make_verification_status(is.na(PMID_manual) | is.na(paper_title_manual), is.na(source_design_file), TRUE, TRUE),
    scoring_status = "verified_metadata_only_not_scored",
    expected_imrs_behavior,
    paper_result_alignment_rule,
    imrs_result_interpretation = classify_interpretation(dataset_id, manuscript_interpretation_group, delta_mean_imrs_z, time_h, expected_imrs_behavior),
