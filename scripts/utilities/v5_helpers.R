...
# final_manuscripts/revised_plots_v6. It intentionally keeps plot geoms, themes,
...
load_old_generator_env_v5 <- function(old_script, project_root, v5_root) {
  ...
  v2_root <- file.path(project_root, "v2_manuscript")
  if (!file.exists(file.path(v2_root, "tables", "dataset_classification_authoritative.tsv"))) {
    candidate <- file.path(project_root, "old_version_plots", "v2_manuscript")
    if (file.exists(file.path(candidate, "tables", "dataset_classification_authoritative.tsv"))) {
      v2_root <- candidate
    }
  }
  extra_results <- file.path(project_root, "manuscript", "figures",
                             "grouped_from_scripts_corrected", "final_text_checked",
                             "publication_extra_generated", "results")
  if (!file.exists(file.path(extra_results, "label_permutation_null_summary.tsv"))) {
    candidate <- file.path(project_root, "05_score", "publication_extra_analyses", "results")
    if (file.exists(file.path(candidate, "label_permutation_null_summary.tsv"))) {
      extra_results <- candidate
    }
  }
...
render_figure1a_merged_workflow_v5 <- function(project_root, v5_root, dpi = 400) {
  workflow_script <- file.path(v5_root, "scripts", "R", "build_merged_imrs_workflow_v5.R")
...
render_v5_panels <- function(project_root, input_root, v5_root, dpi = 400) {
  ...
  old_script <- file.path(project_root, "final_manuscripts", "revised_plots", "scripts",
                          "00_generate_all_reorganized_figures_revised.R")
  old_env <- load_old_generator_env_v5(old_script, project_root, v5_root)
  ...
  for (i in seq_len(nrow(run_specs))) {
    ...
    if (identical(fn_name, "render_Figure1A_merged_workflow_v5")) {
      ...
      paths <- render_figure1a_merged_workflow_v5(project_root, v5_root, dpi)
...
validate_v5_outputs <- function(v5_root, manifest, baseline_v2, baseline_v3, baseline_v4) {
  ...
  v4_path_hits <- manifest_files[grepl("revised_plots_v4", manifest_files, fixed = TRUE)]
  ...
  check_names <- c(
    ...
    "no_outputs_point_to_revised_plots_v4",
    "revised_plots_v2_not_modified",
    "revised_plots_v3_not_modified",
    "revised_plots_v4_not_modified",
  ...
  check_status <- c(
    ...
    "PASS",
    "PASS",
    if (length(v4_path_hits) == 0) "PASS" else "FAIL",
  ...
  check_detail <- c(
    ...
    if (length(v4_path_hits) == 0) "No manifest output path points to revised_plots_v4." else paste(v4_path_hits, collapse = "; "),
...
write_generation_log_v5 <- function(v5_root, generated_files, checks) {
  lines <- c(
    ...
    "Confirmation: revised_plots_v2, revised_plots_v3, and revised_plots_v4 were not modified during v5 generation."
  )
