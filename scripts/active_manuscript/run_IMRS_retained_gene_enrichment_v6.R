#!/usr/bin/env Rscript
# Priority 3 retained IMRS gene-program enrichment analysis for revised_plots_v6.
#
...
try_install_package <- function(pkg) {
  ...
  if (!requireNamespace("BiocManager", quietly = TRUE)) {
    log_msg("Package missing and BiocManager unavailable: ", pkg, ". Install command: ", install_commands[[pkg]])
    return(FALSE)
  }
  log_msg("Package missing; attempting installation: ", pkg)
  ok <- tryCatch({
    if (pkg %in% c("clusterProfiler", "org.Mm.eg.db", "ReactomePA")) {
    ...
    log_msg("Package installation failed for ", pkg, ": ", conditionMessage(e))
    FALSE
  })
  if (!ok) log_msg("Install command for missing package: ", install_commands[[pkg]])
  ok
}
