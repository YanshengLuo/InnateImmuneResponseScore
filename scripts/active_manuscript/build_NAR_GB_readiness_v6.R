...
code_statement <- paste(
  "Analysis code for the frozen, transfer-oriented IMRS scoring framework, figure generation, robustness analyses, comparator-signature benchmarking, retained-gene enrichment analysis, and supplementary table generation will be made available at [GitHub repository URL] and archived at [Zenodo DOI or equivalent archival DOI] upon publication. The repository will include scripts used for metadata processing, locked-anchor coefficient construction, frozen scoring, delivery-versus-control split evaluation, manuscript role curation, robustness checks, figure generation, and supplementary table packaging. Versioned release information and software/session details are provided in the reproducibility materials.",
  "",
  "For peer review, code is available at [private repository link or reviewer-access archive], with the v6 analysis outputs corresponding to the submitted manuscript version.",
  sep = "\n"
)
...
data_statement <- paste(
  paste0("All raw sequencing datasets analyzed in this study were obtained from public repositories, primarily GEO, under the accession numbers listed in Supplementary Table S1. Reprocessed metadata, split-design files, frozen IMRS coefficient tables, ", metric_label, " summaries, robustness outputs, enrichment outputs, and manuscript-ready supplementary tables will be provided at [repository/Zenodo DOI]. The derived data package includes dataset-level provenance, split-level contrast definitions, manuscript role assignments, late or context-shifted boundary-context annotations, robustness summaries, and retained-gene enrichment results. No new human-subject data were generated for this study."),
  "",
  "If raw count matrices cannot be redistributed because they are already hosted by GEO or other repositories, provide scripts and accession numbers sufficient to retrieve and rebuild the analysis inputs.",
  sep = "\n"
)
...
checklist <- tibble::tribble(
  ...
  "Code availability statement candidate", "complete", "NAR_GB_readiness/IMRS_code_availability_statement_candidate.txt", "Contains publication and peer-review placeholders.",
  "Data availability statement candidate", "complete", "NAR_GB_readiness/IMRS_data_availability_statement_candidate.txt", "Contains repository/Zenodo placeholder and raw-data redistribution note.",
...
review_inventory <- tibble::tribble(
  ...
  "Code availability statement", "NAR_GB_readiness/IMRS_code_availability_statement_candidate.txt", "Manuscript code-availability candidate.", "no", "availability statements", "yes", "repository URL and archival DOI placeholders", "Manual action needed before submission.",
  "Data availability statement", "NAR_GB_readiness/IMRS_data_availability_statement_candidate.txt", "Manuscript data-availability candidate.", "no", "availability statements", "yes", "repository/Zenodo DOI placeholder", "Manual action needed before submission.",
...
gap_audit <- tibble::tribble(
  ...
  "Code availability statement", "Candidate created.", "NAR_GB_readiness/IMRS_code_availability_statement_candidate.txt", "manual_action_needed", "Replace GitHub and archival DOI placeholders.", "Peer-review private link also needed if repo is private.",
  "Data availability statement", "Candidate created.", "NAR_GB_readiness/IMRS_data_availability_statement_candidate.txt", "manual_action_needed", "Replace repository/Zenodo DOI placeholder.", "Raw GEO data should be accessed by accession when redistribution is not appropriate.",
...
log_msg("Manual actions remaining: replace repository URL, reviewer-access link, and archival DOI placeholders; confirm raw-data redistribution/retrieval policy; consider creating renv.lock or environment.yml.")
